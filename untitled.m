...
if iex == 5
    ...
    for ii=1:length(xvec)
        ...
        if(ii~=1) 
            yvec(ii)=yvec(ii-1)+dydx(xvec(ii-1),yvec(ii-1))*(xvec(ii)-xvec(ii-1));
        end
    ...
    f = dydx;
    k1 = @(x,y) deltax * dydx(x, y);
    k2 = @(x,y) deltax * dydx(x+deltax/2, y+k1(x,y)/2);
    k3 = @(x,y) deltax * dydx(x+deltax/2, y+k2(x,y)/2);
    k4 = @(x,y) deltax * dydx(x+deltax, y+k3(x,y));
    
    % loop over x
    % compute y(ii+1)
    for ii=1:length(xvec)
        if(ii~=1) 
            yvec(ii)=yvec(ii-1)+(k1(xvec(ii-1),yvec(ii-1))+2*k2(xvec(ii-1),yvec(ii-1))+k4(xvec(ii-1),yvec(ii-1))+2*k3(xvec(ii-1),yvec(ii-1)))/6;
        end
